...
public class Replicator {
	...
	public void replicateZaak(String zaakidentificatie) {
		...
		if (zgwZaak == null) {
			...
			var zdsUrl = this.converter.getZaakService().configService.getConfiguratie().getReplication()
					.getGeefZaakdetails().getUrl();
			var zdsSoapAction = this.converter.getZaakService().configService.getConfiguratie().getReplication()
			...
			ZdsZakLa01GeefZaakDetails zakLa01 = (ZdsZakLa01GeefZaakDetails) XmlUtils
					.getStUFObject(zdsResponse.getBody().toString(), ZdsZakLa01GeefZaakDetails.class);
		...
		// altijd de controle of de documenten al bestaan
		{
			...
			for (ZdsHeeftRelevant relevant : zakZakLa01.antwoord.object.heeftRelevant) {
				...
				log.info("GeefZaakdocumentenLezen voor zaak:" + zaakidentificatie + " met document:"
						+ zaakdocumentidentificatie);
				/*
				 * zdsUrl = this.converter.getZaakService().configService.getConfiguratie().
				 * getReplication().getGeefLijstZaakdocumenten().getUrl(); zdsSoapAction =
				 * this.converter.getZaakService().configService.getConfiguratie().
				 * getReplication().getGeefLijstZaakdocumenten().getSoapaction(); zdsRequest =
				 * new ZdsReplicateGeefLijstZaakdocumentenLv01(); zdsRequest.stuurgegevens =
				 * this.converter.getZdsDocument().stuurgegevens; zdsRequest.parameters = new
				 * ZdsParameters(); //zdsRequest.parameters.setSortering("0");
				 * zdsRequest.parameters.setIndicatorVervolgvraag("false"); zdsRequest.gelijk =
				 * new ZdsZaak(); zdsRequest.gelijk.identificatie = zaakidentificatie;
				 * zdsRequest.scope = new ZdsScope(); zdsRequest.scope.object = new ZdsZaak();
				 * zdsRequest.scope.object.heeftRelevant = new ZdsHeeftRelevant();
				 * zdsRequest.scope.object.heeftRelevant.gerelateerde = new ZdsZaakDocument();
				 *
				 * var zdsResponse = this.converter.getZaakService().zdsClient.post(zdsUrl,
				 * zdsSoapAction, zdsRequest); // fetch the zaak details
				 * log.info("GeefLijstZaakdocumenten response:" + zdsResponse); var zakZakLa01 =
				 * (ZdsZakLa01LijstZaakdocumenten)
				 * XmlUtils.getStUFObject(zdsResponse.getBody().toString(),
				 * ZdsZakLa01LijstZaakdocumenten.class);
				 */
			}
		}
